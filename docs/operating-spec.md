...
Scheduling CTA (all steps): `suggested_date` and `suggested_time` are generated fresh per send, not fixed at template-authoring time, since each step goes out on a different date (Day 1 through Month 5). `calendly_link` is the standing booking link:
`https://calendly.com/sonakshin/ai-commerce-agents-manifest-ai?utm_source=outbound&utm_medium=email&utm_campaign=us_outbound_engine&utm_content=book_call`
